...
//Bool variable for notifi server when set config
int8_t set_config = 0;
...
char* GenData(Data myData, time_Data mytimeData, bool type)
{
    ...
    sprintf(str_SetConfig, "%d", myData.SetConfig);
    ...
    strcat(JSON_buff, "\"SetConf\":\"");
    strcat(JSON_buff, str_SetConfig);
    strcat(JSON_buff, "\",");
...
void mqtt_event_data_callback(char *topic, char *mess)
{
    ...
    if(strstr(topic, "config"))
    {
        if(Json_parse(mess)) 
        {
            ...
            //Ack message
            mqtt_publish_data(config_buff, "ok");
...
void Mqtt_communication_task( void * pvParameters)
{
    ...
    const TickType_t xTime = 30*1000/portTICK_RATE_MS;
    ...
    while(1)
    {
        ...
        xSemaphoreTake( Sensor_Semaphore, portMAX_DELAY );
        Data myData = {ph_value, tds_value, temp_value, ph_set_value, tds_set_value, ph_deadband_value, tds_deadband_value, Nutri_A, Nutri_B, Nu_ratio, Acid_So, Base_So, Water, change_nutri, set_config};
        ...
        Nutri_A = 0; Nutri_B = 0; Acid_So = 0; Base_So = 0; Water = 0; change_nutri = 0; set_config = 0;
...
void Pum_ctr_task(void * pvParameters)
{
    ...
    const TickType_t xTime = 10*60*1000/portTICK_PERIOD_MS;
    xLastWakeTime = xTaskGetTickCount();
